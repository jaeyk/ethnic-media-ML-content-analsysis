...
wf <- create_word_frequency(tokenized_articles, 95, 20)

wf_gran <- create_word_frequency(tokenized_articles_gran, 95, 10)

write_csv(wf, here("processed_data", "word_frequency.csv"))

write_csv(wf_gran, here("processed_data", "word_frequency_gran.csv"))

wf <- read_csv(here("processed_data", "word_frequency.csv"))

wf_gran <- read_csv(here("processed_data", "word_frequency_gran.csv"))
...
# Filter 
wf <- filter_words(wf)
wf_gran <- filter_words(wf_gran)

# Visualize 

wf_plot <- visualize_wf(wf)
wf_gran_plot <- visualize_wf(wf_gran)
