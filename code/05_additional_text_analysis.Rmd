...
word_frequency <- create_word_frequency(tokenized_articles, 90, 30)

word_frequency_gran <- create_word_frequency(tokenized_articles_gran, 90, 15)
```

# Visualize 

```{r}
wf_plot <- visualize_wf(word_frequency) + labs(title = "Maximum threshold")

wf_gran_plot <- visualize_wf(word_frequency_gran) + labs(title = "Minimum threshold")

wf_plot / wf_gran_plot

ggsave(here("outputs", "relative_word_freq.png"), width = 15, height = 10)
```