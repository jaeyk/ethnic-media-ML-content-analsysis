...
group_folds <- groupKFold(dataset$year, k = 10)
...
result <- list(data_df_train, data_df_test, response_train, group_folds)
...
svm_cm <- confusionMatrix(svm_pred, ground_truth)
...
balanced_acu_results <- c(
  bal_accuracy_vec(svm_pred, ground_truth),
...
mod_asian_lp <- build_test(asian_lp, linked_progress)
