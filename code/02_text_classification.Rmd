...
labeled_asian$text <- clean_text(labeled_asian) 
labeled_black$text <- clean_text(labeled_black) 
...
predict_unlabeled <- function(dataset, measure){
  
  dataset$text <- clean_text(dataset) 
  
  model_ready <- train_test_response(dataset, measure)
  
  data_df_train <- model_ready[[1]] 
  data_df_test <- model_ready[[2]]
  response_train <- model_ready[[3]]
  ground_truth <- model_ready[[4]]
...
### 4.2. Applying the prediction function to each data

```{r}

asian_lp_full <- predict_unlabeled(unlabeled_asian, asian_lp)
asian_lh_full <- predict_unlabeled(unlabeled_asian, asian_lh)

black_lp_full <- predict_unlabeled(unlabeled_black, black_lp)
black_lh_full <- predict_unlabeled(unlabeled_black, black_lh)
