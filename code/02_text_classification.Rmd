...
Importing data. I skipped the first column of all of these files because they are not informative.
...
tidy_docs <- dataset %>%
  ...
  group_by(word) %>% 
  filter(n() > 10) %>% # remove words appeared less than 10 times
  ungroup()
  
# Create two random samples stratifying on year variable (the exactly same variable I used to create training samples for content anlysis)
...
print(paste("N of observations:", dim(train_matrix)[1], "," , "N of features:", dim(train_matrix)[2]))
...
print(paste("N of observations:", dim(test_matrix)[1], "," , "N of features:", dim(test_matrix)[2]))
...
train_joined <- tibble(doc_id = as.integer(rownames(train_matrix))) %>%
  left_join(dataset %>%
              dplyr::select(doc_id, measure))

# This is the target 

train_response <- train_joined$linked_progress == 1
...
test_response <- test_joined$measure == 1
...
trctrl <- trainControl(method = "repeatedcv", 
             repeats = 5) # five repeats of 10-fold CV 
            # classProbs = FALSE,
            # sampling = "down") # Down the size of the class to match wit hthe 
...
nb_cm <- confusionMatrix(nb_pred, target_response)

################# PENALIZED LOGIT ####################

cv <- cv.glmnet(x = as.matrix(train_matrix),
                y = as.factor(train_response), 
                alpha = 1, 
                family = "binomial",
                parallel = TRUE,
...
lasso_mod <- train(x = train_matrix,
                  y = as.factor(train_response), 
                  method = "glmnet",
                  trControl = trctrl,
...
lasso_cm <- confusionMatrix(lasso_pred, target_response)
...
logitboost_cm <- confusionMatrix(logitboost_pred, target_response)
...
rf_cm <- confusionMatrix(rf_pred, target_response)
...
balanced_acu_results <- c(
  bal_accuracy_vec(nb_pred, target_response),
  bal_accuracy_vec(lasso_pred, target_response),
  bal_accuracy_vec(logitboost_pred, target_response),
  bal_accuracy_vec(rf_pred, target_response))
