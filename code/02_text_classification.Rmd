...
# 6 is the index number of linked progress column and 7 is the index number of linked hurt.

asian_lp <- vectorize_train_test(labeled_asian, 6)

asian_lh <- vectorize_train_test(labeled_asian, 7)

black_lp <- vectorize_train_test(labeled_black, 6)

black_lh <- vectorize_train_test(labeled_black, 7)
...
trctrl <- trainControl(method = "repeatedcv", repeats = 5,
                     classProbs = TRUE,
                  #   summaryFunction = twoClassSummary, # I commented this out to use Accuracy as a metric 
                     sampling = "smote") # Create artificial observations for the minority class
