...
write.csv(irr_summary, "/home/jae/content-analysis-for-evaluating-ML-performances/outputs/irr_summary.csv")
```
...
train_asian_plot <- df %>%
  ...
  gather(type, value, linked_progress, linked_hurt) %>%
  filter(value == 1) %>%
  filter(topic != "Mismatch") %>%
  group_by(type) %>%
  count(topic) %>%
  ggplot(aes(x = fct_reorder(topic, n), y = n, fill = type)) +
    geom_col(position = "dodge") +
    coord_flip() +
    theme_base() +
    labs(x = "Topics", y = "N", title = "N of agreed topics",
...
ggarrange(train_asian_plot, train_black_plot, ncol = 2, nrow =1, common.legend = TRUE)
...
df %>%
  dplyr::select(linked_progress, linked_hurt, group) %>%
  group_by(group) %>%
  summarize(corr = cor(linked_progress, linked_hurt), method = "Pearson") %>%
  ggplot(aes(x = fct_reorder(group, corr), y = corr)) +
    geom_col() +
    labs(title = "Correlation Coefficients between Linked Progress and Linked Hurt",
