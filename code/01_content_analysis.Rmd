...
sample_asian <- bind_cols(sample_asian, training_asian %>% select(linked_progress, linked_hurt, Topics_C))
sample_black <- bind_cols(sample_black, training_black %>% select(linked_progress, linked_hurt, Topics_C))
...
sample_asian_text <- sample_asian %>%
  filter(Topics_C == "Political representation" & linked_progress == 1 & linked_hurt == 0) %>%
  select(date, source, text) 

sample_black_text <- sample_black %>%
  filter(Topics_C == "Criminal justice" & linked_hurt == 1 & linked_progress == 0) %>%
  select(date, source, text) 
