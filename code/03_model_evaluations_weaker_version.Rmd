---
title: "Model evaluations"
author: "Jae Yeon Kim"
output:
html_document: 
  toc: true
  theme: united
---

## 0. Setup 

```{r}

# Clean up the environment

rm(list = ls())

# Import libraries (adapted from this link: https://stackoverflow.com/questions/4090169/elegant-way-to-check-for-missing-packages-and-install-them)

if (!require("pacman")) install.packages("pacman")
pacman::p_load(
        tidyverse, # for the tidyverse framework
        ggpubr, # for arranging ggplot2 
        ggthemes, # for fancy ggplot themes
        broom # for visualizing coefficients
)

source("/home/jae/content-analysis-for-evaluating-ML-performances/functions/theme_publications.r")

theme_set(theme_Publication(14))

source("/home/jae/content-analysis-for-evaluating-ML-performances/functions/text_analysis.r")

```

## 1. Importing files 

```{r include=FALSE}

# From ML models 

# None 

asian_lp_models <- read_csv("/home/jae/content-analysis-for-evaluating-ML-performances/processed_data/asian_lp_models_gran.csv")[,-1]
asian_lh_models <- read_csv("/home/jae/content-analysis-for-evaluating-ML-performances/processed_data/asian_lh_models_gran.csv")[,-1]

black_lp_models <- read_csv("/home/jae/content-analysis-for-evaluating-ML-performances/processed_data/black_lp_models_gran.csv")[,-1]
black_lh_models <- read_csv("/home/jae/content-analysis-for-evaluating-ML-performances/processed_data/black_lh_models_gran.csv")[,-1]

# Resampled 

asian_lp_models_resample <- read_csv("/home/jae/content-analysis-for-evaluating-ML-performances/processed_data/asian_lp_models_resample_gran.csv")[,-1]
asian_lh_models_resample <- read_csv("/home/jae/content-analysis-for-evaluating-ML-performances/processed_data/asian_lh_models_resample_gran.csv")[,-1]

black_lp_models_resample <- read_csv("/home/jae/content-analysis-for-evaluating-ML-performances/processed_data/black_lp_models_resample_gran.csv")[,-1]
black_lh_models_resample <- read_csv("/home/jae/content-analysis-for-evaluating-ML-performances/processed_data/black_lh_models_resample_gran.csv")[,-1]

# From content analysis: Correlation coefficients and Kappa statistics 

irr_summary <- read_csv("/home/jae/content-analysis-for-evaluating-ML-performances/processed_data/irr_summary.csv")[,-1]

```
## 2. Wrangling data

### 2.1. Putting all ML evals together

```{r}
  
# Put all 
all_evals <- bind_rows(mutate(asian_lp_models, group = "Asian Americans", measure = "Linked progress", resampling = "None"),
          mutate(asian_lh_models, group = "Asian Americans", measure = "Linked hurt", resampling = "None"),
          mutate(black_lp_models, group = "African Americans", measure = "Linked progress", resampling = "None"),
          mutate(black_lh_models, group = "African Americans", measure = "Linked hurt", resampling = "None"),
          mutate(asian_lp_models_resample, group = "Asian Americans", measure = "Linked progress", resampling = "Upsampling"),
          mutate(asian_lh_models_resample, group = "Asian Americans", measure = "Linked hurt", resampling = "Upsampling"),
          mutate(black_lp_models_resample, group = "African Americans", measure = "Linked progress", resampling = "Upsampling"),
          mutate(black_lh_models_resample, group = "African Americans", measure = "Linked hurt", resampling = "Upsampling")
          )

# Gather all the models 
all_evals <- all_evals %>%
  gather(metrices, rate, c("Accuracy", "Balanced Accuracy"))

```

### 2.2. Renaming the columns in the content analysis 

```{r}

irr_resummary <- irr_summary %>%
  rename(measure = type, 
         content_agreement = percent_agreement, 
         content_kappa = kappa)

```

### 2.3. Merging them together 

```{r}

# Merging 
merged_model <- all_evals %>% right_join(irr_resummary) 

# Upper casing col names 
names(merged_model) <- tolower(names(merged_model))

write.csv(merged_model, "/home/jae/content-analysis-for-evaluating-ML-performances/processed_data/merged_model50.csv")
```

## 3. Visualizing data 

### 3.1. Resampling effect  

```{r}

no_resampling_plot <- merged_model %>%
  filter(resampling == "None") %>%
  mutate(rate = round(rate, 3)) %>%
  ggplot(aes(x = models, y = rate, fill = metrices)) +
    geom_col(position = "dodge") +
    scale_fill_manual(name = "Metrics", labels = c("Accuracy","Balanced accuracy"), values=c("red","blue")) +
    facet_grid(measure ~ group) +
    ylim(c(0, 1)) +
    labs(title = "ML performances (without resampling)", x = "Models", y = "Rate",
         fill = "Metrics") +
    scale_y_continuous(labels = scales::percent_format(accuracy = 5L)) +
    geom_text(aes(label = paste(rate*100, "%")), position=position_dodge(width=0.9), vjust=-0.25) 

resampling_plot <- merged_model %>%
  filter(resampling != "None") %>%
  mutate(rate = round(rate, 3)) %>%
  ggplot(aes(x = models, y = rate, fill = metrices)) +
    geom_col(position = "dodge") +
    scale_fill_manual(name = "Metrics", labels = c("Accuracy","Balanced accuracy"), values=c("red","blue")) +
    facet_grid(measure ~ group) +
    ylim(c(0, 1)) +
    labs(title = "ML performances (with upsampling)", x = "Models", y = "Rate",
         fill = "Metrics") +
    scale_y_continuous(labels = scales::percent_format(accuracy = 5L)) +
    geom_text(aes(label = paste(rate*100, "%")), position=position_dodge(width=0.9), vjust=-0.25) 

ggarrange(no_resampling_plot, resampling_plot, ncol = 1, nrow = 2)

ggsave("/home/jae/content-analysis-for-evaluating-ML-performances/outputs/ml_performances_gran.png", height = 17, width = 10)

```

```{r}

no_resampling_plot <- merged_model %>%
  filter(resampling == "None") %>%
  mutate(rate = round(rate, 3)) %>%
  ggplot(aes(x = fct_reorder(models, rate), y = rate, fill = metrices)) +
    geom_col(position = "dodge") +
    scale_fill_manual(name = "Metrics", labels = c("Accuracy","Balanced accuracy"), values=c("red","blue")) +
    geom_hline(aes(yintercept = content_agreement), linetype = "dashed") +
    facet_grid(measure ~ group) +
    ylim(c(0, 1)) +
    labs(title = "ML performances (with upsampling)", x = "Models", y = "Rate",
         fill = "Metrics") +
    scale_y_continuous(labels = scales::percent_format(accuracy = 5L)) +
    geom_text(aes(label = paste(rate*100, "%")), position=position_dodge(width=0.9), vjust=-0.25) 


resampling_plot <- merged_model %>%
  filter(resampling != "None")  %>%
  mutate(rate = round(rate, 3)) %>%
  ggplot(aes(x = fct_reorder(models, rate), y = rate, fill = metrices)) +
    geom_col(position = "dodge") +
    scale_fill_manual(name = "Metrics", labels = c("Accuracy","Balanced accuracy"), values=c("red","blue")) +
    geom_hline(aes(yintercept = content_agreement), linetype = "dashed") +
    facet_grid(measure ~ group) +
    ylim(c(0, 1)) +
    labs(title = "ML performances (with upsampling)", x = "Models", y = "Rate",
         fill = "Metrics") +
    scale_y_continuous(labels = scales::percent_format(accuracy = 5L)) +
    geom_text(aes(label = paste(rate*100, "%")), position=position_dodge(width=0.9), vjust=-0.25) 

ggarrange(no_resampling_plot, resampling_plot, ncol = 1, nrow = 2)

ggsave("/home/jae/content-analysis-for-evaluating-ML-performances/outputs/ml_content_gran.png", height = 17, width = 10)
```

